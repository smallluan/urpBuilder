...
export const CODE_EDITOR_JS_GLOBAL_COMPLETIONS: Completion[] = [
  ...
  { label: 'Boolean', type: 'class' },
  {
    ...
    detail: '流程代码节点 · 函数参数',
    info:
      '沙箱内不可直接使用 window / document / fetch / localStorage；请用 dataHub.storage（持久化与会话）、dataHub.runtime（网络/定时器/日志）及原有 message、router、cloud 等。',
  },
  {
    label: 'dataHub.message',
    type: 'function',
    detail: '(content, options?) => void',
    info: "函数式消息提示，示例：dataHub.message('保存成功')。",
  },
  {
    label: 'dataHub.message.success',
    type: 'function',
    detail: '(content, options?) => void',
    info: "成功提示，示例：dataHub.message.success('发布成功')。",
  },
  {
    label: 'dataHub.message.warning',
    type: 'function',
    detail: '(content, options?) => void',
    info: "警告提示，示例：dataHub.message.warning('请先选择记录')。",
  },
  {
    label: 'dataHub.message.error',
    type: 'function',
    detail: '(content, options?) => void',
    info: "错误提示，示例：dataHub.message.error('提交失败')。",
  },
  {
    label: 'dataHub.dialog',
    type: 'variable',
    detail: '函数式对话框上下文',
    info: '支持 alert/confirm，不需要拖拽 Dialog 组件。',
  },
  {
    label: 'dataHub.dialog.alert',
    type: 'function',
    detail: '(contentOrOptions) => Promise<void>',
    info: "简单提示弹窗，示例：await dataHub.dialog.alert('操作完成')。",
  },
  {
    label: 'dataHub.dialog.confirm',
    type: 'function',
    detail: '(contentOrOptions) => Promise<boolean>',
    info: "确认弹窗，示例：const ok = await dataHub.dialog.confirm('确认删除？')。",
  },
  {
  ...
  {
    label: 'dataHub.storage',
    type: 'variable',
    detail: 'memory · session · local',
    info: '按 scope 隔离：memory 仅内存暂存；session / local 对应 Web Storage，值支持 JSON 序列化对象。登录态等请用 dataHub.storage.local.set。',
  },
  {
    label: 'dataHub.storage.memory.get',
    type: 'function',
    detail: '(key) => unknown',
    info: '读取本会话内存缓存（刷新即失）。',
  },
  {
    label: 'dataHub.storage.memory.set',
    type: 'function',
    detail: '(key, value) => void',
    info: '写入本会话内存缓存。',
  },
  {
    label: 'dataHub.storage.session.get',
    type: 'function',
    detail: '(key) => unknown',
    info: '读取 sessionStorage（同标签页内有效）。',
  },
  {
    label: 'dataHub.storage.session.set',
    type: 'function',
    detail: '(key, value) => void',
    info: '写入 sessionStorage。',
  },
  {
    label: 'dataHub.storage.local.get',
    type: 'function',
    detail: '(key) => unknown',
    info: '读取 localStorage（可存登录态等，键已按预览 scope 加前缀）。',
  },
  {
    label: 'dataHub.storage.local.set',
    type: 'function',
    detail: '(key, value) => void',
    info: '写入 localStorage。',
  },
  {
    label: 'dataHub.runtime',
    type: 'variable',
    detail: 'fetch · console · setTimeout …',
    info: '白名单宿主能力：请用 dataHub.runtime.fetch 而非全局 fetch；定时器与 console 亦在此。',
  },
  {
    label: 'dataHub.runtime.fetch',
    type: 'function',
    detail: '(input, init?) => Promise<Response>',
    info: '与原生 fetch 等价，沙箱内全局 fetch 已禁用。',
  },
  {
    label: 'dataHub.runtime.console',
    type: 'variable',
    detail: 'log / info / warn / error / debug',
    info: '调试输出，等价于 console。',
  },
  {
    label: 'dataHub.runtime.setTimeout',
    type: 'function',
    detail: '同 window.setTimeout',
    info: '沙箱内请使用此处，勿依赖全局 setTimeout（已建议走白名单）。',
  },
  {
  ...
  {
    label: "dataHub.message('保存成功')",
    type: 'snippet',
    detail: '函数式消息提示',
    info: '无需放置 Message 组件，可在任意代码节点直接调用。',
  },
  {
    label: "const ok = await dataHub.dialog.confirm('确认执行该操作？'); if (!ok) return;",
    type: 'snippet',
    detail: '函数式确认弹窗',
    info: '常用于删除、发布等危险操作前二次确认。',
  },
  {
